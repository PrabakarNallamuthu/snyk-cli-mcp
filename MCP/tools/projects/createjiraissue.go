...
func CreatejiraissueHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/org/%s/project/%s/issue/%s/jira-issue", cfg.BaseURL, issueId, orgId, projectId)
...
func CreateCreatejiraissueTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_org_orgId_project_projectId_issue_issueId_jira-issue",
		mcp.WithDescription("Create jira issue"),
		mcp.WithString("issueId", mcp.Required(), mcp.Description("The issue ID to create Jira issue for.")),
		...
		mcp.WithObject("fields", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreatejiraissueHandler(cfg),
	}
}
