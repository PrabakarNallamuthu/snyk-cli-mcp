...
func ApplyingattributesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/org/%s/project/%s/attributes", cfg.BaseURL, orgId, projectId)
...
func CreateApplyingattributesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_org_orgId_project_projectId_attributes",
		mcp.WithDescription("Applying attributes"),
		...
		mcp.WithArray("lifecycle", mcp.Description("")),
		mcp.WithArray("criticality", mcp.Description("")),
		mcp.WithArray("environment", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ApplyingattributesHandler(cfg),
	}
}
