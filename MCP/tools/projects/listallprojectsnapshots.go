...
func ListallprojectsnapshotsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/org/%s/project/%s/history%s", cfg.BaseURL, orgId, projectId, queryString)
...
func CreateListallprojectsnapshotsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_org_orgId_project_projectId_history",
		mcp.WithDescription("List all project snapshots"),
		mcp.WithString("orgId", mcp.Required(), mcp.Description("The organization ID. The `API_KEY` must have access to this organization.")),
		mcp.WithString("projectId", mcp.Required(), mcp.Description("The project ID to return snapshots for.")),
		mcp.WithString("perPage", mcp.Description("The number of results to return (the default is 10, the maximum is 100).")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListallprojectsnapshotsHandler(cfg),
	}
}
