...
func ListallprojectissuepathsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["snapshotId"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("snapshotId=%v", val))
		}
		...
		url := fmt.Sprintf("%s/org/%s/project/%s/issue/%s/paths%s", cfg.BaseURL, orgId, projectId, issueId, queryString)
...
func CreateListallprojectissuepathsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_org_orgId_project_projectId_issue_issueId_paths",
		mcp.WithDescription("List all project issue paths"),
		...
		mcp.WithString("snapshotId", mcp.Description("The project snapshot ID for which to return issue paths. If set to `latest`, the most recent snapshot will be used. Use the \"List all project snapshots\" endpoint to find suitable values for this.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListallprojectissuepathsHandler(cfg),
	}
}
