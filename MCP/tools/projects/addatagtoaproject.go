...
func AddatagtoaprojectHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/org/%s/project/%s/tags", cfg.BaseURL, orgId, projectId)
...
func CreateAddatagtoaprojectTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_org_orgId_project_projectId_tags",
		mcp.WithDescription("Add a tag to a project"),
		mcp.WithString("orgId", mcp.Required(), mcp.Description("The organization ID. The `API_KEY` must have access to this organization.")),
		mcp.WithString("projectId", mcp.Required(), mcp.Description("The project ID to apply the tag to")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    AddatagtoaprojectHandler(cfg),
	}
}
