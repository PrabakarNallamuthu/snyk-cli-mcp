...
func GetgrouplevelauditlogsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/group/%s/audit%s", cfg.BaseURL, groupId, queryString)
...
func CreateGetgrouplevelauditlogsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_group_groupId_audit",
		mcp.WithDescription("Get group level audit logs"),
		mcp.WithString("groupId", mcp.Required(), mcp.Description("The group ID. The `API_KEY` must have access to this group.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetgrouplevelauditlogsHandler(cfg),
	}
}
