...
func Get_test_pip_packagename_versionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/test/pip/%s/%s%s", cfg.BaseURL, packageName, version, queryString)
...
func CreateGet_test_pip_packagename_versionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_test_pip_packageName_version",
		mcp.WithDescription("Test for issues in a public package by name and version"),
		mcp.WithString("packageName", mcp.Required(), mcp.Description("The package name.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_test_pip_packagename_versionHandler(cfg),
	}
}
