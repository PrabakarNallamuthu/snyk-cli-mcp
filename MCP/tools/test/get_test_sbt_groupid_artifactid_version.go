...
func Get_test_sbt_groupid_artifactid_versionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/test/sbt/%s/%s/%s%s", cfg.BaseURL, groupId, artifactId, version, queryString)
...
func CreateGet_test_sbt_groupid_artifactid_versionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_test_sbt_groupId_artifactId_version",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_test_sbt_groupid_artifactid_versionHandler(cfg),
	}
}
