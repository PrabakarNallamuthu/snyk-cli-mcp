...
func ListalldependenciesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/org/%s/dependencies%s", cfg.BaseURL, orgId, queryString)
...
func CreateListalldependenciesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_org_orgId_dependencies",
		mcp.WithDescription("List all dependencies"),
		...
		mcp.WithString("page", mcp.Description("The page of results to fetch.")),
		mcp.WithString("perPage", mcp.Description("The number of results to fetch per page (maximum is 1000).")),
		mcp.WithObject("filters", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListalldependenciesHandler(cfg),
	}
}
