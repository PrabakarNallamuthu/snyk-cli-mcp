...
func DeletecredentialsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/org/%s/integrations/%s/authentication", cfg.BaseURL, orgId, integrationId)
...
func CreateDeletecredentialsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_org_orgId_integrations_integrationId_authentication",
		mcp.WithDescription("Delete credentials"),
		mcp.WithString("orgId", mcp.Required(), mcp.Description("The organization ID. The `API_KEY` must have access to this organization.")),
		mcp.WithString("integrationId", mcp.Required(), mcp.Description("The integration ID.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletecredentialsHandler(cfg),
	}
}
