...
func CreateawebhookHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/org/%s/webhooks", cfg.BaseURL, orgId)
...
func CreateCreateawebhookTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_org_orgId_webhooks",
		mcp.WithDescription("Create a webhook"),
		...
		mcp.WithString("secret", mcp.Description("Input parameter: This is a password you create, that Snyk uses to sign our transports to you, so you be sure the notification is authentic. Your `secret` should: Be a random string with high entropy; Not be used for anything else; Only known to Snyk and your webhook transport consuming code;")),
		mcp.WithString("url", mcp.Description("Input parameter: Webhooks can only be configured for URLs using the `https` protocol. `http` is not allowed.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreateawebhookHandler(cfg),
	}
}
